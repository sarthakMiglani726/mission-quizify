...
class QuizGenerator:
        ...
        for _ in range(self.num_questions):
            ...
            for attempt in range(i):
                try:
                    ...
                    if question_str.strip() == "":  # Check if the returned string is empty
                        raise ValueError("Received empty string from question generation.")
                    question_dict = json.loads(question_str)
                    if self.validate_question(question_dict):
                        self.question_bank.append(question_dict)
                        break
                    elif attempt == i - 1:
                        print(f"Failed to generate unique question after {i} attempts.")
                except (ValueError, json.JSONDecodeError) as e:
                    print(f"Error during question generation: {str(e)}")
                    if attempt == i - 1:
                        print("Skipping question due to repeated errors.")
    ...
    def validate_question(self, question: dict) -> bool:
        ...
        if question_text in self.seen_questions:
            return False
